            ...
            Route::middleware(['Ban', 'Country', 'Blocked', 'CheckStatus', 'Tfa'])->group(function () {
                Route::middleware(['Banks'])->group(function () {
                    Route::middleware(['Kyc'])->group(function () {
                        ...
                        //USSD
                        Route::post('ussd', [UserController::class, 'ussdRequestHandler']);
                        Route::get('hi', [UserController::class, 'my_ussd']);
                        //End USSD
                        //Mpesa
                            Route::post('register-url', [UserController::class, 'registerURLS']);
                            Route::post('get-token', [UserController::class, 'getMpesaAccessToken']);
                            Route::get('mobile', [UserController::class, 'mpesa'])->name('user.mpesa.index');
                            Route::post('mobile',[UserController::class, 'createVoucher'])->name('user.mpesa.index');
                            Route::post('sendmobile', [UserController::class, 'business2Customer'])->name('user.mpesa.index');

                         //End

                         //MTN
                         Route::get('mtn', [UserController::class, 'mtnTransactions'])->name('user.mtn.index');
                         Route::post('mtn-pay', [MTNController::class, 'makePayment']);
                         Route::get('mpesa', [UserController::class, 'mpesa'])->name('user.mpesa.index');
                         Route::post('mpesa',[UserController::class, 'createVoucher'])->name('user.mpesa.index');
                      //End


                          //Nedbank
                          Route::get('nedbank', [UserController::class, 'nedbank'])->name('user.nedbank.index');
              
                         //End
                        //stitch
                        Route::get('stitch', [UserController::class, 'get_stitch_access_token']);
                        Route::get('stitch', [UserController::class, 'show_stitch']);
                        Route::get('getpubkey', [UserController::class, 'get_stitch_public_key']);
                        Route::get('stichme', [UserController::class, 'getStitchToken']);
                        Route::get('snitches', function(){
                        ...
                        Route::get('luno', [UserController::class, 'lunoAuth'])->name('lunoAuth');
                        Route::get('lunoBalance', [UserController::class, 'getLunoBalance'])->name('getLunoBalance');
                        Route::get('showluno', [UserController::class, 'showLuno'])->name('showLuno');
                        Route::post('cryptobuy', [UserController::class, 'cryptobuy'])->name('cryptobuy');
                        //endLuno
...
Route::group(['prefix' => 'admin', 'middleware' => 'auth:admin'], function () {
    ...
    //Since I can't run php artisan on console I'll run it on the route
