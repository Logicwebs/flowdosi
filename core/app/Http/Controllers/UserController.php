...
use App\Http\Controllers\UssdMenuTrait;
...
class UserController extends Controller
{
    ...
    //
        ...
        public function lunoAuth()
        {
            $ch = curl_init();

            curl_setopt($ch, CURLOPT_URL, 'https://api.mybitx.com/api/1/ticker?pair=ETHXBT');
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');

            curl_setopt($ch, CURLOPT_USERPWD, env('LUNO_KEY_ID') . ':' . env('LUNO_SECRET') );
        ...
        public function getLunoBalance()
        {
            try{
            $ch = curl_init();
        ...
        public function showLuno()
        {
            $luno_cred = json_decode($this->lunoAuth());
            return view('user.luno.index')->with('data', $luno_cred);
        }
...
//        $payment_method == 'bank transfer' ?  = :  
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, "pair=XBTZAR&type=BUY&counter_volume=100.50");
        curl_setopt($ch, CURLOPT_USERPWD, env('LUNO_KEY_ID')  . ':' .  env('LUNO_SECRET'));
    ...
    //Fund account
     ...
     //stitch
        ...
        public function get_stitch_public_key()
        {
            $pub_key = openssl_pkey_get_private(file_get_contents(env('PEM_FILE')));
        ...
        public function getStitchToken(){
            // $payload = [
            ...
            $factory = JWTFactory::customClaims([
                ...
                "iss"=> env('CLIENT_ID'),
                "sub" => env('CLIENT_ID'),
                "jti" => '16456467ae8048a44e3be609601293c34af2bf6852',
            ]);
        ...
        public function show_stitch()
        {
            $user=User::find(Auth::user()->id);
           // $var1 = get_stitch_public_key();

          return view('user.stitch.index', compact($user));
         // return $var1;
    ...
    public function mtnTransactions(){
        ...
        return view('user.mtn.index', compact($user));
    }
    //USSD Transactions

    public function my_ussd()
    {
        $response  = 'CON, '. 'CON dear customer, please choose from the menu below';
        $response .= "1. withdraw a voucher \n";
        $response .= "2. Create a voucher";
        ...
        switch ($level) {
            case ($level == 1 && !empty($ussd_string)):
                if ($ussd_string_exploded[0] == "1") {
                    ...
                    $this->ussd_proceed("CON Kindly input your pin");
                } else if ($ussd_string_exploded[0] == "2") {
                    //If user selected 2, end session
                    $this->ussd_stop("CON Thank you for reaching out to SampleUSSD.");
                } else {
                    $this->ussd_stop("CON Invalid Input");
                }
            break;
            case 2:
            ...
            case 3:
                if ($ussd_string_exploded[2] == "1") {                   
                    $this->ussd_stop("CON You will receive an sms shortly.");
                    $this->sendText("CON You have successfully subscribed to updates from SampleUSSD.",$phone);
                } else if ($ussd_string_exploded[2] == "2") {
                    $this->ussd_stop("CON You will receive more information on SampleUSSD via sms shortly.");
                    $this->sendText("This is a subscription service from SampleUSSD.",$phone);
                } else if ($ussd_string_exploded[2] == "3") {
                    $this->ussd_stop("CON Thanks for reaching out to SampleUSSD.");              
                } else {
                    $this->ussd_stop("CON Invalid input!");
                }
            break;
        ...
        switch ($level) {
            case ($level == 1 && !empty($ussd_string)):
                if ($ussd_string_exploded[0] == "1") {
                    ...
                    $this->ussd_proceed("CON Please enter your full name and desired pin separated by commas. \n eg: Jane Doe,1234");
                } else if ($ussd_string_exploded[0] == "2") {
                    //If user selected 2, send them the information
                    $this->ussd_stop("CON You will receive more information on SampleUSSD via sms shortly.");
                    $this->sendText("CON This is a subscription service from SampleUSSD.",$phone);
    ...
    */
    ...
    public function ussdLogin($details, $phone)
    {
        ...
        if ($user->pin == $details ) {
            ...
            return $this->ussd_stop("CON Login was unsuccessful!");
        }
    }
    ...
    public function mpesa(){
        ...
        return view('user.mpesa.index', compact($user));
    }
    //accessTokens
    public function getMpesaAccessToken()
    {
         $url = env('MPESA_ENV') == 0
         ? 'https://sandbox.safaricom.co.ke/oauth/v1/generate?grant_type=client_credentials'
         : 'http://api.safaricom.co.ke/oauth/v1/generate?grant_type=client_credentials';
    ...
    //Mepsa B2C
    public function business2customer(Request $request)
    { 
        $user=Auth::guard('user')->user()->id;
        // $ch = curl_init('https://sandbox.safaricom.co.ke/mpesa/b2c/v1/paymentrequest');
        // curl_setopt($ch, CURLOPT_HTTPHEADER, [
        //     'Authorization: Bearer O2YMVWKxZw3EKXcG7bgelGsy3Vrn', 
        //     'Content-Type: application/json'
        // ]);
        // curl_setopt($ch, CURLOPT_POST, 1);
        $b2cResponse=Mpesa::b2c($request->amount, $request->phone,'BusinessPayment',$request->remark);
        // $params = array(
        //     //ternary initiation of the name
        //     "InitiatorName" => $request->name ? $request->name : $user->first_name . ' ' . $user->last_name,
        //     "SecurityCredential" => "",
    ...
    public function createVoucher(Request $request)
    {
        $parameters = array(
            ...
            'CallBackURL' =>  "http://192.168.64.3/script/user/nedbank",
            'AccountReference' =>  $request->account_reference,
            'TransactionDesc' =>  'transaction by: ' . Session::get('name')
          );
    ...
    public function createNedbankWallet()
    {
    //create wallet
     $curl = curl_init();

     curl_setopt_array($curl, array(
     CURLOPT_URL => "https://api.nedbank.co.za/apimarket/sandbox/nb-wallet/wallets",
     CURLOPT_RETURNTRANSFER => true,
     CURLOPT_ENCODING => "",
     CURLOPT_MAXREDIRS => 10,
     CURLOPT_TIMEOUT => 30,
     CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
     CURLOPT_CUSTOMREQUEST => "POST",
     CURLOPT_POSTFIELDS => "{\"CallbackUrl\":\"http://gekimta.ws/ifku\",\"ExternalUniqueId\":\"5628752860020736\",\"FirstName\":\"Pearl\",\"LastName\":\"Mayer\",\"IdentityNumber\":\"762142187847680\",\"Msisdn\":\"cifi\",\"IncomeSourceType\":\"fedizcicsosu\",\"IndustryType\":\"paavanuljiwh\",\"OccupationType\":\"343585384648008\"}",
     CURLOPT_HTTPHEADER => array(
     ...
     $data['response_error'] = $err;

     return view('user.nedbank.profile');

    }
//Nedbank screen
public function nedbank(){
    ...
    return view('user.nedbank.index', $data);
}

//Nedbank Mobile Money
